...
CREATE TABLE `orders` (
  ...
  `shipping_address` TEXT NOT NULL COMMENT '收货地址(JSON格式)',
...
CREATE TABLE `menus` (
  ...
  `external` TINYINT DEFAULT 0 COMMENT '是否外链',
  `cache` TINYINT DEFAULT 0 COMMENT '是否缓存',
  `permission_id` BIGINT COMMENT '权限ID',
  `type` TINYINT DEFAULT 1 COMMENT '菜单类型：1目录，2菜单，3按钮',
  `button_key` VARCHAR(100) COMMENT '按钮标识',
