...
export class MigrationController {
  ...
  async addSystemPermissionsToSuperAdmin() {
    try {
      ...
      // 获取所有系统管理权限（ID 62-78）
      const systemPermissionIds = [62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78];
      
      // 找出缺少的系统权限
      const missingPermissionIds = systemPermissionIds.filter(id => !currentPermissionIds.includes(id));
      ...
      // 获取缺少的权限实体
      const missingPermissions = await this.permissionRepository.findByIds(missingPermissionIds);
      console.log('找到的缺少权限:', missingPermissions.map(p => ({ id: p.id, code: p.code })));

      // 合并权限：现有权限 + 缺少的系统权限
      const allPermissions = [...superAdminRole.permissions, ...missingPermissions];
      ...
      for (const permissionId of missingPermissionIds) {
        try {
          ...
          console.log(`成功添加权限 ${permissionId} 到角色 ${superAdminRole.id}`);
        } catch (error) {
      ...
      return {
        ...
        data: {
          ...
          totalPermissions: currentPermissionIds.length + missingPermissionIds.length
        },
  ...
  async createMediaPermissions() {
    try {
      ...
      const mediaPermissions = [
        { name: '资源', code: 'media', type: 'menu' as const, description: '媒体资源管理模块' },
        { name: '静态资源', code: 'media:static', type: 'menu' as const, description: '静态资源管理', parentCode: 'media' },
        { name: '查看静态资源', code: 'media:static:view', type: 'button' as const, description: '查看静态资源列表', parentCode: 'media:static' },
        { name: '上传静态资源', code: 'media:static:upload', type: 'button' as const, description: '上传静态资源', parentCode: 'media:static' },
        { name: '删除静态资源', code: 'media:static:delete', type: 'button' as const, description: '删除静态资源', parentCode: 'media:static' },
      ];
      ...
      for (const permData of mediaPermissions) {
        if (permData.parentCode) {
          ...
          if (permission && parent) {
            permission.parentId = parent.id;
            await this.permissionRepository.save(permission);
            console.log(`建立父子关系: ${permData.code} -> ${permData.parentCode}`);
          }
        }
      }
  ...
  async checkRoleMenusTable() {
    try {
      ...
      // 检查数据量
      const countResult = await queryRunner.query(`SELECT COUNT(*) as count FROM role_menus`);
  ...
  async assignAllMenusToSuperAdmin() {
    try {
      ...
      if (roleMenus.length > 0) {
        const values = roleMenus.map(rm => 
          `(${rm.role_id}, ${rm.menu_id}, NOW(), NOW())`
        ).join(', ');
      ...
      // 5. 统计分配结果
      const menusByType = allMenus.reduce((acc, menu) => {
        const type = menu.type === 1 ? '目录' : menu.type === 2 ? '菜单' : '按钮';
  ...
  async checkSuperAdminMenus() {
    try {
      ...
      // 3. 统计菜单类型
      const menuStats = roleMenus.reduce((acc, rm) => {
        const type = rm.menu_type === 1 ? '目录' : rm.menu_type === 2 ? '菜单' : '按钮';
        acc[type] = (acc[type] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);

      // 4. 获取所有菜单总数进行对比
      const totalMenus = await this.menuRepository.count({ where: { status: 1 } });
      ...
      return {
        ...
        data: {
          ...
          menus: roleMenus.map(rm => ({
            ...
            menuType: rm.menu_type === 1 ? '目录' : rm.menu_type === 2 ? '菜单' : '按钮',
