...
async function safeFixBrandsTable() {
  ...
  try {
    ...
    if (missingFields.length === 0) {
      ...
      // 4. 保存SQL到文件
      fs.writeFileSync('backend/generated-fix.sql', alterSql + '\n');
      console.log('\n✅ SQL已保存到 backend/generated-fix.sql');
      ...
      console.log('\n📝 建议手动执行以下步骤:');
      console.log('1. 备份数据库');
      console.log('2. 执行 backend/generated-fix.sql 中的SQL语句');
      console.log('3. 运行 node check-brands-table.js 验证修复结果');
    }
...
WHERE merchantId IS NULL OR merchantId = 0 OR iconUrl = '';
    ...
    fs.writeFileSync('backend/migrate-brands-data.sql', migrationSql);
    console.log('✅ 数据迁移SQL已保存到 backend/migrate-brands-data.sql');
...
FROM brands;
    ...
    fs.writeFileSync('backend/verify-brands-fix.sql', verifySql);
    console.log('✅ 验证SQL已保存到 backend/verify-brands-fix.sql');

    console.log('\n🎯 修复准备完成！');
    console.log('\n📋 生成的文件:');
    console.log('  - backend/generated-fix.sql (表结构修复)');
    console.log('  - backend/migrate-brands-data.sql (数据迁移)');
    console.log('  - backend/verify-brands-fix.sql (验证脚本)');
    
    console.log('\n🔧 执行步骤:');
    console.log('1. 备份数据库');
    console.log('2. mysql -h 43.139.80.246 -u root -p wechat_mall < backend/generated-fix.sql');
    console.log('3. mysql -h 43.139.80.246 -u root -p wechat_mall < backend/migrate-brands-data.sql');
    console.log('4. node check-brands-table.js (验证结果)');
